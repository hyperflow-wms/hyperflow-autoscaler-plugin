...
describe('ResourceRequests class', function() {
  context('Exported util for memory conversion to bytes', function() {
    ...
    it('handle SI units', function() {
      ...
      expect(RR.Utils.parseMemString("1P")).to.equal(1000*1000*1000*1000*1000);
      expect(RR.Utils.parseMemString("1E")).to.equal(1000*1000*1000*1000*1000*1000);
    });
    ...
    it('handle power-of-2 units', function() {
      ...
      expect(RR.Utils.parseMemString("1Ti")).to.equal(1024*1024*1024*1024);
      expect(RR.Utils.parseMemString("1Pi")).to.equal(1024*1024*1024*1024*1024);
  ...
  context('Exported util for cpu conversion to millis', function() {
    ...
    it('throw error for invalid formats', function() {
      ...
      expect(() => { RR.Utils.parseCpuString("10Kii"); }).to.throw(Error);
      expect(() => { RR.Utils.parseCpuString("0-0"); }).to.throw(Error);
    });
  });
